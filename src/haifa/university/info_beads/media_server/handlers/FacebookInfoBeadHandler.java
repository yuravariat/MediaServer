...
import haifa.university.info_beads.inputs.facebook.*;
import haifa.university.info_beads.inputs.youtube.PlaylistItem;
import haifa.university.mediaserver.common.AppLogger;

/**
 * FacebookInfoBeadHandler
 * @author Yuri Variat
 * @version  1.0;
 *
 */
public class FacebookInfoBeadHandler extends InfoBead<ArrayList<Triplet<FBLike>>> implements Runnable {
	private static final String TAG="FacebookInfoBeadHandler";
	private static final long serialVersionUID = 1L;
	private File applicationDir = new File(Paths.get(".").toAbsolutePath().normalize().toString(), "MediaAgent");
	private final String SETTINGS_FILE_NAME = "FacebookInfoBeadHandler";
	private boolean debugMode = true;

    private volatile boolean running = false;
    private volatile boolean started = false;

    public FacebookInfoBeadHandler(){
        super();
        initialize();
    }
	...
	{
		// -------------- set id ------------------------
		this.setInfoBeadId("FacebookInfoBeadHandler");
		...
		metadata.setInfoBeadName("FacebookInfoBeadHandler");
	...
	public void handleData(Triplet<ArrayList<Triplet<FBLike>>> inTriplet) 
	{
		ArrayList<Triplet<FBLike>> collection = inTriplet.getInfoItem().getInfoValue();
	}
	public void pushData(Triplet<ArrayList<Triplet<FBLike>>> tripletToDeliver) {
